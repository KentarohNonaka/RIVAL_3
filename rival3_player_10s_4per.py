'''
rival_3_player_10s_4%.py
Copyright (C) 2024 Kentaroh Nonaka
...
This program has been modified by Kentaroh Nonaka on 2024-05-27.
'''
...
class MCTSPlayer(BasePlayer):
    # USIエンジンの名前
    name = 'RIVAL_3_10s_4%'        #変更
    # デフォルトチェックポイント
    DEFAULT_MODELFILE = '/content/RIVAL_3/GCTdenryu_0001-035.pth'
...
#以下変更
    ...
    # 手加減手法の実装
    # 相手の平均勝率に近い手から選択確率が最も高い手を選ぶ(該当なしの場合，選択確率が最も高い手を優先)，強引な手加減の抑制
    def get_bestmove_and_print_pv(self):
        ...
        for i in win_sort_idx:
            #print(win_rate[i])
            if win_rate_diff[i] <= 0.1 and current_node.policy[i] >= 0.04: #勝率差10％以内かつ選択確率4％以上
                near = True
                terms = 1
        ...
        if near:  #勝率差10％以内かつ選択確率5％の手以上がある
            pass

        else:     #勝率差10％以内かつ選択確率5％以上の手がなければ勝率差15%以内かつ選択確率X%以上の手を探す
            for i in win_sort_idx:
                #print(win_rate[i])
                if win_rate_diff[i] <= 0.15 and current_node.policy[i] >= 0.075:  #勝率差15%以内かつ選択確率X%以上
                    near = True
                    terms = 2
                    #print(win_rate_diff[i], current_node.policy[i], i)
                    if current_node.policy[i] > current_node.policy[selected_index]:
        ...
        if near:  #勝率差15％以内かつ選択確率X％以上の手がある
            pass

        else:     #勝率差15％以内かつ選択確率X％以上の手がなければ勝率差20%以内かつ選択確率Y%以上の手を探す
            for i in win_sort_idx:
                #print(win_rate[i])
                if win_rate_diff[i] <= 0.2 and current_node.policy[i] >= 0.1:  #勝率差20%以内かつ選択確率Y%以上
                    near = True
                    terms = 3
                    #print(win_rate_diff[i], current_node.policy[i], i)
                    if current_node.policy[i] > current_node.policy[selected_index]:
        ...
        #該当する手がない場合，すべての候補手から選択確率が最大の手を選択
        else:
        ...
        """        if win_rate[i] >= 0.85:  #勝率85%以上の候補手があればその中で選択確率が最大の手を選択
                selected_index = win_sort_idx[0]
                print(selected_index)
                print(f'win_sort_idx\n{win_sort_idx}\n')
                for i in win_sort_idx:
                    if win_rate[i] >= 0.85 and current_node.policy[i] > current_node.policy[selected_index]:
                        selected_index = i
                        print(selected_index)
                    elif win_rate[i] < 0.85:
                        break
            else:
                pass"""
        ...
        print(f'win_rate_diff:{win_rate_diff[selected_index]}')
